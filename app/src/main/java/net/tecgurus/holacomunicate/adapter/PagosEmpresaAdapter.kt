...
/**
 * Created By:
 * @author Abraham Casas Aguilae
 */
class PagosEmpresaAdapter(val context: Context, val layout: Int, val list: List<Pagos>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        val view: View
        val vh: PagosEmpresaViewHolder
        if (convertView == null) {
            view = parent!!.inflate(layout)
            vh = PagosEmpresaViewHolder(view)
            view.tag = vh
        } else {
            view = convertView
            vh = view.tag as PagosEmpresaViewHolder
        }
...
class PagosEmpresaViewHolder(view: View) {
